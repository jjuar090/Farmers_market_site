// Utility functions to read and parse the farmers markets CSV data
// This file contains helper functions to work with the CSV data
...
export interface FarmersMarket {
  id: string; // We'll generate this from the market name
  state_name: string;
...
export function getFarmersMarkets(): FarmersMarket[] {
  try {
...
export function getFeaturedMarkets(count: number = 6): FarmersMarket[] {
  const allMarkets = getFarmersMarkets();
  return allMarkets.slice(0, count);
}

// Function to search markets by name or location
export function searchMarkets(query: string): FarmersMarket[] {
  const allMarkets = getFarmersMarkets();
